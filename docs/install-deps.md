...
```bash
jr gh:just-buildit/just-bashit/src/install-deps.sh < deps.toml
```
...
```bash
jr gh:just-buildit/just-bashit/src/install-deps.sh --template deps.toml
```
...
```bash
jr gh:just-buildit/just-bashit/src/install-deps.sh < deps.toml
jr gh:just-buildit/just-bashit/src/install-deps.sh -g runtime,dev < deps.toml
```
...
```bash
jr gh:just-buildit/just-bashit/src/install-deps.sh@abc1234 < deps.toml
```

### Thin project shim

Projects that want a one-step `./install-deps.sh` without requiring
`jr` on the caller's `PATH` can include a shim:
...
if command -v jr >/dev/null 2>&1; then
    ...
    echo "error: just-runit (jr) not found." >&2
    exit 1
fi
```
